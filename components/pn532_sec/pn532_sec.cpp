...
// Based on:
// - https://cdn-shop.adafruit.com/datasheets/PN532SECC106_Application+Note_v1.2.pdf
...
static const char *const TAG = "pn532_sec";

void PN532SEC::setup() {
  ESP_LOGCONFIG(TAG, "Setting up PN532SEC...");

  // Get version data
  if (!this->write_command_({PN532SEC_COMMAND_VERSION_DATA})) {
  ...
  if (!this->read_response(PN532SEC_COMMAND_VERSION_DATA, version_data)) {
  ...
  if (!this->write_command_({
          PN532SEC_COMMAND_SAMCONFIGURATION,
          0x01,  // normal mode
  ...
  if (!this->read_response(PN532SEC_COMMAND_SAMCONFIGURATION, wakeup_result)) {
  ...
  if (!this->read_response(PN532SEC_COMMAND_SAMCONFIGURATION, sam_result)) {
...
void PN532SEC::update() {
  for (auto *obj : this->binary_sensors_)
    obj->on_scan_end();

  if (!this->write_command_({
          PN532SEC_COMMAND_INLISTPASSIVETARGET,
          0x01,  // max 1 card
...
void PN532SEC::loop() {
  ...
  bool success = this->read_response(PN532SEC_COMMAND_INLISTPASSIVETARGET, read);
  ...
  if (read.size() < 6U + nfcid_length) {
    // oops, pn532_sec returned invalid data
    return;
  }
...
bool PN532SEC::write_command_(const std::vector<uint8_t> &data) {
  ...
  // TFI (Frame Identifier, 0xD4 means to PN532SEC, 0xD5 means from PN532SEC)
...
void PN532SEC::send_nack_() {
  ESP_LOGV(TAG, "Sending NACK for retransmit");
  this->write_data({0x00, 0x00, 0xFF, 0xFF, 0x00, 0x00});
  delay(10);
}

void PN532SEC::turn_off_rf_() {
  ESP_LOGV(TAG, "Turning RF field OFF");
  this->write_command_({
      PN532SEC_COMMAND_RFCONFIGURATION,
      0x01,  // RF Field
      0x00,  // Off
  });
}

std::unique_ptr<nfc::NfcTag> PN532SEC::read_tag_(std::vector<uint8_t> &uid) {
...
bool PN532SEC::format_tag_(std::vector<uint8_t> &uid) {
  uint8_t type = nfc::guess_tag_type(uid.size());
  if (type == nfc::TAG_TYPE_MIFARE_CLASSIC) {
...
bool PN532SEC::write_tag_(std::vector<uint8_t> &uid, nfc::NdefMessage *message) {
  uint8_t type = nfc::guess_tag_type(uid.size());
  if (type == nfc::TAG_TYPE_MIFARE_CLASSIC) {
...
float PN532SEC::get_setup_priority() const { return setup_priority::DATA; }

void PN532SEC::dump_config() {
  ESP_LOGCONFIG(TAG, "PN532SEC:");
...
bool PN532SECBinarySensor::process(std::vector<uint8_t> &data) {
