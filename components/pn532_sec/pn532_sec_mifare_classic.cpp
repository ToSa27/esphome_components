...
static const char *const TAG = "pn532_sec.mifare_classic";

std::unique_ptr<nfc::NfcTag> PN532SEC::read_mifare_classic_tag_(std::vector<uint8_t> &uid) {
...
bool PN532SEC::read_mifare_classic_block_(uint8_t block_num, std::vector<uint8_t> &data) {
  if (!this->write_command_({
          PN532SEC_COMMAND_INDATAEXCHANGE,
          0x01,  // One card
          nfc::MIFARE_CMD_READ,
          block_num,
      })) {
  ...
  if (!this->read_response(PN532SEC_COMMAND_INDATAEXCHANGE, data) || data[0] != 0x00) {
    return false;
  }
...
bool PN532SEC::auth_mifare_classic_block_(std::vector<uint8_t> &uid, uint8_t block_num, uint8_t key_num,
                                       const uint8_t *key) {
  std::vector<uint8_t> data({
      PN532SEC_COMMAND_INDATAEXCHANGE,
      0x01,       // One card
  ...
  if (!this->read_response(PN532SEC_COMMAND_INDATAEXCHANGE, response) || response[0] != 0x00) {
...
bool PN532SEC::format_mifare_classic_mifare_(std::vector<uint8_t> &uid) {
...
bool PN532SEC::format_mifare_classic_ndef_(std::vector<uint8_t> &uid) {
...
bool PN532SEC::write_mifare_classic_block_(uint8_t block_num, std::vector<uint8_t> &write_data) {
  std::vector<uint8_t> data({
      PN532SEC_COMMAND_INDATAEXCHANGE,
      0x01,  // One card
  ...
  std::vector<uint8_t> response;
  if (!this->read_response(PN532SEC_COMMAND_INDATAEXCHANGE, response)) {
    ESP_LOGE(TAG, "Error writing block %d", block_num);
    return false;
  }
...
bool PN532SEC::write_mifare_classic_tag_(std::vector<uint8_t> &uid, nfc::NdefMessage *message) {
