...
static const char *const TAG = "pn532_sec.mifare_ultralight";

std::unique_ptr<nfc::NfcTag> PN532SEC::read_mifare_ultralight_tag_(std::vector<uint8_t> &uid) {
...
bool PN532SEC::read_mifare_ultralight_page_(uint8_t page_num, std::vector<uint8_t> &data) {
  ...
  // We only want 1 page of data but the PN532SEC returns 4 at once.
...
bool PN532SEC::is_mifare_ultralight_formatted_() {
  std::vector<uint8_t> data;
...
uint16_t PN532SEC::read_mifare_ultralight_capacity_() {
  std::vector<uint8_t> data;
...
bool PN532SEC::find_mifare_ultralight_ndef_(uint8_t &message_length, uint8_t &message_start_index) {
  std::vector<uint8_t> data;
...
bool PN532SEC::write_mifare_ultralight_tag_(std::vector<uint8_t> &uid, nfc::NdefMessage *message) {
...
bool PN532SEC::clean_mifare_ultralight_() {
  uint32_t capacity = this->read_mifare_ultralight_capacity_();
...
bool PN532SEC::write_mifare_ultralight_page_(uint8_t page_num, std::vector<uint8_t> &write_data) {
  std::vector<uint8_t> data({
      PN532SEC_COMMAND_INDATAEXCHANGE,
      0x01,  // One card
