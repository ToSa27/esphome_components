...
static const char *const TAG = "pn532_sec_spi";

void PN532SECSpi::setup() {
  ESP_LOGI(TAG, "PN532SECSpi setup started!");
  this->spi_setup();
...
bool PN532SECSpi::write_data(const std::vector<uint8_t> &data) {
  this->enable();
  delay(2);
...
bool PN532SECSpi::read_data(std::vector<uint8_t> &data, uint8_t len) {
  ...
  while (true) {
    ...
    if (millis() - start_time > 100) {
      ESP_LOGV(TAG, "Timed out waiting for readiness from PN532SEC!");
      return false;
    }
    yield();
  }

  // Read data (transmission from the PN532SEC to the host)
  this->enable();
  delay(2);
...
bool PN532SECSpi::read_response(uint8_t command, std::vector<uint8_t> &data) {
  ...
  bool valid_header = (static_cast<uint8_t>(header[3] + header[4]) == 0 &&  // LCS, len + lcs = 0
                       header[5] == 0xD5 &&        // TFI - frame from PN532SEC to system controller
...
void PN532SECSpi::dump_config() {
  PN532SEC::dump_config();
