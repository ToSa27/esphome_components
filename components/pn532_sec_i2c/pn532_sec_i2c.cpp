...
static const char *const TAG = "pn532_sec_i2c";

bool PN532SECI2C::write_data(const std::vector<uint8_t> &data) {
  return this->write(data.data(), data.size()) == i2c::ERROR_OK;
}

bool PN532SECI2C::read_data(std::vector<uint8_t> &data, uint8_t len) {
  delay(1);
...
bool PN532SECI2C::read_response(uint8_t command, std::vector<uint8_t> &data) {
  ...
  bool valid_header = (static_cast<uint8_t>(data[4] + data[5]) == 0 &&  // LCS, len + lcs = 0
                       data[6] == 0xD5 &&                               // TFI - frame from PN532SEC to system controller
...
uint8_t PN532SECI2C::read_response_length_() {
